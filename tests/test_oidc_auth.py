...
def test_media_auth_providers_default_to_redirect_provider() -> None:
    async def run() -> None:
        engine = create_async_engine("sqlite+aiosqlite:///:memory:", future=True)
        async with engine.begin() as conn:
            await conn.run_sync(Base.metadata.create_all)
        ...
        async with session_maker() as db_session:
            ...
            db_session.add_all([jellyfin, plex])
            await db_session.commit()
            await db_session.refresh(jellyfin)
            await db_session.refresh(plex)

            response = await auth_routes.list_media_auth_login_providers(db_session)

            assert response.default_service_config_id == plex.id
            assert [provider.auth_mode for provider in response.providers] == [
                "credentials",
                "redirect",
            ]
