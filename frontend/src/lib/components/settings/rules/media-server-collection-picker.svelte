<script lang="ts">
  ...
  let items = $state<MediaServerCollectionLookup[]>([]);
  let page = $state(1);
  ...
  const loadCollections = async (
    ...
    try {
      ...
      const resp = await get_api<PaginatedMediaServerCollectionsResponse>(
        `/api/rules/media-server-collections?${params.toString()}`,
      );
      page = resp.page;
      ...
      error = e.message ?? "Failed to load media-server collections.";
      if (!append) items = [];
    } finally {
      loading = false;
      loadingMore = false;
...
<Dialog.Root bind:open>
  ...
  >
    <Dialog.Header class="px-6 pt-5 pb-3 shrink-0 border-b border-border">
      <Dialog.Title>Select Media-Server Collections</Dialog.Title>
      <Dialog.Description>
        Search local Plex, Jellyfin, and Emby collections and apply them to this
        rule condition.
      </Dialog.Description>
    </Dialog.Header>
