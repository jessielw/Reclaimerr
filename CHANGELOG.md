...
- Automatic deletion could hand an item an endless review period. Reclaimerr only told your media server about a delete when **Allow Media Server Fallback Deletion** was on, but that setting is about whether Reclaimerr may delete _through_ the media server - not about whether the library should be kept in step afterwards. With it off, Radarr, Sonarr, or Reclaimerr itself would remove the files and the media server would carry on serving an entry that no longer had any. The next **Sync Media** run re-imported that entry, the next **Scan Cleanup Candidates** run flagged it again, and because a review period is measured from the moment a candidate is created, a brand new candidate meant the countdown started over. An item could sit at "Eligible now", get actioned, come back a few hours later with a fresh **Flagged** date and another full review period ahead of it, and repeat that indefinitely without ever being deleted. Reclaimerr now always reconciles the media server after files are removed: it deletes the item when the fallback setting allows it, and otherwise asks the server to re-scan the affected path, which is a read-only request that deletes nothing. One caveat if you run Plex: a path scan only drops missing items when Plex's _Empty trash automatically after every scan_ library setting is enabled, which is its default.
- A movie that any disabled or deleted Radarr instance had ever tracked could not be deleted at all, silently. Reclaimerr only rewrites an instance's stored media references when it can reach that instance, so turning a Radarr off left its references behind forever. The candidates view hides references belonging to a disabled instance, so the movie still showed a single **Radarr** link - but deletion routing counted the leftover one, decided the movie lived in two Radarrs, and concluded it could not prove a Radarr delete would remove only the selected file. It fell through to a media-server file delete, which then needed **Allow Media Server Fallback Deletion**, and installs with that off logged `Media server fallback disabled - skipping movie-version deletion` on every single run. Routing now ignores references for instances that are not loaded, matching what the rest of that code path already did, and every sync clears out references belonging to service configurations that are disabled or gone.

### Changed

- Cleanup candidates now show when their deletion has been failing. The backend has always recorded the attempt count and the reason on the candidate, but none of it reached the UI - a candidate that failed every run for a week still just read `Auto delete: Eligible now`, with nothing to suggest anything was wrong. A red **Delete failed** line now sits under the auto-delete line with the number of attempts, when the last one was, and the reason Reclaimerr recorded. Grouped rows report the worst-affected member rather than whichever one happens to sort first. The same three fields are also exposed on `GET /api/v1/candidates`.
- Episode deletions now honor **Allow Media Server Fallback Deletion** like season, series, and movie deletions already did. Removing an episode's file used to delete the episode from the media server regardless of that setting. It still keeps the library in step when the setting is off - it just does it with a path re-scan instead of a delete.
- Reclaimerr says more in the log about two situations it used to pass over in silence. When a version-scoped movie delete cannot be promoted to a Radarr delete, it now names the movie and prints the versions and Radarr references it compared, so the reason is in the log instead of having to be inferred. And when a candidate is flagged again within a fortnight of Reclaimerr acting on the same title, it warns that the review period has restarted and that the media server is the likely reason - the loop described above, made visible.

## [0.4.4] - 2026-09-10

### Changed

- Updated 14 backend dependencies.

### Fixed

- Last air date could become stale and not match correctly on rules.
...
- Promoting a second media server of the _same_ type to main left the previous main flagged as main as well. Only servers of a different type were demoted, so two Plex servers could both carry the flag -- and since every lookup of "the main server" expects a single row, the next media sync and the next save from the media server settings both failed with `MultipleResultsFound`. Promotion now demotes every other media server by identity rather than by type.
...
- **Fixed a false negative in requester watch rules.** The "watched after requesting" bar used the _newest_ request a user had for a movie or season. Seerr writes a separate request row for a 4K copy and for every re-request of an airing season, so a later request moved the bar past plays that had already happened and a season the requester demonstrably finished reported as not watched. The bar is now that requester's earliest request. Request _age_ (`Seerr latest active request`) still tracks the newest request.
...
- Regression: switching the main media server now reliably queues its required full resync
